...
import { StudyPlan, Section } from "@/types";
...
export default function StudyPlanPage() {
  ...
  // Group chapters by section dynamically
  const sectionGroups: Record<string, typeof plan.recommendedChapters> = Object.fromEntries(
    exam.sections.map(s => [s.id, []])
  );

  plan.recommendedChapters.forEach((rec) => {
    const chapter = chapterMap[rec.chapterId];
    if (chapter && sectionGroups[chapter.section]) {
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-8">
        ...
        <div className="mb-8 rounded-lg bg-[#fafafa] border border-[#e5e7eb] p-5">
          ...
          <div className="space-y-2">
            {plan.weakAreas.map((area) => {
              const sectionLabel = exam.sections.find(s => s.id === area.section)?.label || area.section;
              return (
                <div key={area.section} className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-[#374151]">{sectionLabel}</span>
                </div>
              );
            })}
          </div>
      ...
      {exam.sections.map((section) => {
        const chapters = sectionGroups[section.id] || [];
        ...
        return (
          <div key={section.id} className="mb-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-semibold text-[#0d0d0d]">{section.label}</h2>
