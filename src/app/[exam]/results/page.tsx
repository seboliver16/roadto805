...
import { PracticeSession, Theme } from "@/types";
    ...
    <div className="min-h-screen bg-white">
      <header className="border-b border-[#e5e7eb] bg-white">
        <div className="mx-auto flex max-w-3xl items-center justify-between px-4 py-4">
          <button onClick={() => router.push(`${basePath}/dashboard`)} className="text-sm text-[#6b7280] hover:text-[#374151]">
            &larr; Home
          </button>
      ...
      <main className="mx-auto max-w-3xl px-4 py-4">
        ...
        {sectionBreakdown && (
          <div className="mb-8 grid grid-cols-1 gap-3 sm:grid-cols-3">
            {exam.sections.map((section) => {
              const data = sectionBreakdown[section.id];
              if (!data || data.total === 0) return null;
              const pct = Math.round((data.score / data.total) * 100);
              return (
                <div key={section.id} className="rounded-lg bg-[#fafafa] border border-[#e5e7eb] p-4 text-center">
                  <p className="text-sm font-medium text-[#374151]">{section.label}</p>
                ...
                return (
                  <div key={theme} className="rounded-lg bg-[#fafafa] p-4 border border-[#e5e7eb]">
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-[#0d0d0d]">{exam.themes[theme] || theme}</span>
        ...
        <div className="space-y-3">
          {missedCount > 0 && (
            <button
              onClick={() => router.push(`${basePath}/review?session=${sessionId}`)}
          ...
          {weakThemes.length > 0 && (
            <button
              onClick={() => {
                ...
                router.push(`${basePath}/practice?themes=${themeList}`);
              }}
