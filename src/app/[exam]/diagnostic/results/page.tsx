...
function DiagnosticResultsContent() {
  ...
  const handleGeneratePlan = async () => {
    ...
    const sectionScores = session.sectionBreakdown ?? Object.fromEntries(
    ...
    setGenerating(false);
    router.push(`${basePath}/study-plan`);
  };
  ...
  const sectionBreakdown: Partial<Record<string, { score: number; total: number }>> = session.sectionBreakdown ?? {};
    ...
    <main className="mx-auto max-w-3xl px-4 py-4">
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        {exam.sections.map((section) => {
          const data = sectionBreakdown[section.id] ?? { score: 0, total: 0 };
          const percent = data.total > 0 ? Math.round((data.score / data.total) * 100) : 0;

          return (
            <div key={section.id} className="rounded-lg bg-[#fafafa] border border-[#e5e7eb] p-5 text-center">
              <p className="text-sm font-medium text-[#374151] mb-3">{section.label}</p>
