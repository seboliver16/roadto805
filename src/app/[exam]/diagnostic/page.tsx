...
function DiagnosticContent() {
  ...
  const SECTION_ORDER = exam.sections.map(s => s.id);
  ...
  const handleNext = useCallback(async () => {
    if (currentIndex < questions.length - 1) {
      ...
      const sectionBreakdown: Record<string, { score: number; total: number }> = {};
      for (const s of exam.sections) {
        sectionBreakdown[s.id] = { score: 0, total: 0 };
      }
      ...
      setFinished(true);
      router.push(`${basePath}/diagnostic/results?session=${sessionId}`);
    }
  }, [currentIndex, questions, answers, currentQuestion, selectedAnswer, sessionId, user, router, exam, basePath]);
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-white">
        <div className="animate-fade-in text-center">
          ...
          <h2 className="text-3xl font-bold text-[#0d0d0d]">{getSectionLabel(transitionSection)}</h2>
        </div>
      </div>
    );
  }
