...
function SessionReview({ sessionId }: { sessionId: string }) {
  ...
  useEffect(() => {
    ...
    if (missedForTheme.length > 0) {
      generateThemeSummary(exam.themes[topWeakTheme] || topWeakTheme, missedForTheme)
        .then(setThemeSummary)
        .catch(() => setThemeSummary(""));
    }
  }, [weakThemes, reviewItems, exam]);
  ...
  if (reviewItems.length === 0) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4">
        ...
        <button onClick={() => router.push(`${basePath}/dashboard`)} className="rounded-lg bg-[#0d0d0d] px-6 py-3 text-white font-semibold hover:bg-[#1a1a1a]">Back to Home</button>
      </div>
    );
  }
    ...
    <div className="min-h-screen">
      <header className="border-b border-[#e5e7eb] bg-white">
        <div className="mx-auto flex max-w-3xl items-center justify-between px-4 py-4">
          <button onClick={() => router.push(`${basePath}/dashboard`)} className="text-sm text-[#6b7280] hover:text-[#0d0d0d]">&larr; Home</button>
          ...
          <div className="mb-6 rounded-lg bg-[#fafafa] border border-[#e5e7eb] p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {weakThemes.map((theme) => (
                <span key={theme} className="rounded-full bg-[#f3f4f6] px-3 py-1 text-sm text-[#374151]">{exam.themes[theme] || theme}</span>
              ))}
            </div>
          </div>
        )}
          ...
          <div key={currentIndex} className="animate-fade-in">
            <div className="mb-4 flex flex-wrap items-center gap-2">
              <Badge variant={currentItem.question.section === "quant" ? "blue" : currentItem.question.section === "verbal" ? "green" : "purple"}>
                {getSectionLabel(currentItem.question.section)}
              </Badge>
              {currentItem.question.themes.map((theme) => (
                <span key={theme} className="rounded-full bg-[#f3f4f6] px-3 py-1 text-xs font-medium text-[#6b7280]">{exam.themes[theme] || theme}</span>
              ))}
            </div>
            ...
            <div className="flex gap-3">
              ...
              {currentIndex < reviewItems.length - 1 ? (
                ...
                <button onClick={() => router.push(`${basePath}/practice?themes=${weakThemes.join(",")}`)} className="flex-1 rounded-lg bg-[#0d0d0d] py-4 text-lg font-semibold text-white hover:bg-[#1a1a1a]">Drill These Themes</button>
              )}
            </div>
          </div>
        )}
...
function DashboardReview() {
  ...
  const analytics = useMemo(() => computeAnalytics(filteredAttempts, exam), [filteredAttempts, exam]);
  const recommendations = useMemo(() => computeRecommendations(analytics, exam), [analytics, exam]);
  ...
  const getSectionShortLabel = (sectionId: string) =>
    exam.sections.find(s => s.id === sectionId)?.shortLabel || sectionId;

  function handleAiSummary(theme: Theme) {
    if (aiSummaryLoading) return;
    setAiSummaryLoading(true);
    const themeLabel = exam.themes[theme] || theme;
    ...
    generateThemeSummary(themeLabel, missed.slice(0, 5))
      .then((content) => setAiSummary({ theme: themeLabel, content }))
  ...
  if (allAttempts.length === 0) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4">
        ...
        <button onClick={() => router.push(`${basePath}/practice`)} className="rounded-lg bg-[#0d0d0d] px-6 py-3 text-white font-semibold hover:bg-[#1a1a1a]">Start Practicing</button>
      </div>
    );
  }
            ...
            {sectionFilter !== "all" && (
              <>
                ...
                >
                  ...
                  {getQuestionTypesForSection(sectionFilter, exam).map((qt) => (
          ...
          <>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              {exam.sections.map((section) => {
                const data = analytics.sectionBreakdown[section.id];
                if (!data || data.total === 0) return (
                  <div key={section.id} className="rounded-lg bg-[#fafafa] border border-[#e5e7eb] p-5">
                    <h3 className="text-sm font-semibold text-[#0d0d0d]">{section.shortLabel}</h3>
                ...
                return (
                  <div key={section.id} className="rounded-lg bg-[#fafafa] border border-[#e5e7eb] p-5 flex items-center gap-4">
                  ...
                  <div className="space-y-2.5 max-h-80 overflow-y-auto pr-1">
                    {analytics.themeBreakdown.map(({ theme, total, accuracy }) => (
                      <div key={theme} className="flex items-center gap-3">
                        <span className="w-36 text-sm font-medium truncate shrink-0" title={exam.themes[theme] || theme}>{exam.themes[theme] || theme}</span>
                ...
                <div className="space-y-3">
                  {recommendations.map((rec) => (
                    <div key={rec.id} className="flex items-center justify-between rounded-lg bg-white p-4 border border-[#e5e7eb]">
                      ...
                      <button
                        onClick={() => router.push(`${basePath}${rec.practiceUrl}`)}
                    ...
                    {aiSummary ? (
                      ...
                      >
                        {aiSummaryLoading ? (
                          ...
                          `Get AI analysis for ${exam.themes[analytics.themeBreakdown[0].theme] || analytics.themeBreakdown[0].theme}`
                        )}
                      </button>
                    )}
                  </div>
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex flex-wrap items-center gap-2 mb-1.5">
                                <Badge variant={question.section === "quant" ? "blue" : question.section === "verbal" ? "green" : "purple"} size="sm">
                                  {getSectionShortLabel(question.section)}
                                </Badge>
                                ...
                                {question.themes.slice(0, 2).map((t) => (
                                  <span key={t} className="text-xs text-[#9ca3af]">{exam.themes[t] || t}</span>
                                ))}
                              </div>
