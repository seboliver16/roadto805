    ...
    return (
      <main className="mx-auto max-w-3xl px-4 py-12 bg-white">
        <div className="animate-fade-in rounded-lg bg-white border border-[#e5e7eb] p-10 text-center">
          ...
          <h2 className="text-3xl font-bold text-[#0d0d0d]">Welcome to {exam.name}!</h2>
          <p className="mt-3 text-[#374151] max-w-lg mx-auto leading-relaxed">
            Start with a diagnostic test to assess your strengths across all {exam.sections.length} {exam.shortName} sections.
        ...
        <div className="mb-8 grid grid-cols-1 gap-3 sm:grid-cols-3 animate-fade-in">
          {exam.sections.map((section) => {
            ...
            return (
              <button
                key={section.id}
                onClick={() => router.push(`${basePath}/practice?mode=review&section=${section.id}`)}
                className="rounded-xl border border-[#e5e7eb] bg-white p-5 text-left hover:bg-[#fafafa] transition-colors group"
              >
                <div className="flex items-center justify-between mb-3">
                  <p className="text-sm font-semibold text-[#6b7280] uppercase tracking-wide">{section.shortLabel}</p>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4 animate-fade-in">
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-[#6b7280]">Full timed {exam.shortName} simulation</p>
            </div>
          </button>
          <button
            onClick={() => {
